...
class DecisionTree(object):
    ...
    def _entropy(self, y):
        ...
        if len(y) == 0:
            ...
            pc1 = len(y[y==np.unique(y)[0]]) / float(len(y))
            pc2 = 1 - pc1
            return - (pc1 * np.log2(pc1) + pc2 * np.log2(pc2))
    ...
    def _information_gain(self, y, y1, y2):
        ...
        return self.impurity_criterion(y) - float(len(y1)) / len(y) * self.impurity_criterion(y1) - float(len(y2)) / len(y) * self.impurity_criterion(y2)
    ...
    def _choose_split_index(self, X, y):
        ...
        splits = []
        gains = []
        for split_index in range(num_features):
            for split_value in np.unique(X[:, split_index]):
                X1, y1, X2, y2 = self._make_split(X, y, split_index, split_value)
                gain = self._information_gain(y, y1, y2)
                index.append(split_index)
                value.append(split_value)
                splits.append([X1, y1, X2, y2])
                gains.append(gain)
        ...
        if max_gain == 0:
            ...
            return index[max_gain_index], value[max_gain_index], splits[max_gain_index]
